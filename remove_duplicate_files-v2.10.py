...
book = xlwt.Workbook()
sheet = book.add_sheet('重复文件DuplicatedFiles')
folderfilelist = []
folderfilelist2: Any = []
time1 = 0
time2 = 0
...
while True:
    ...
    if h == "1":
        ...
        while 1:
            ...
            if time3 != 1:
                folderlen = str(input("请问你要继续输入目录么？[y|n]\nDo you want to continue entering the directory?"))
            else:
                folderlen = "y"
            if folderlen == "y":
                ...
                print("这次发现了%d个新文件,总共%d个" % (len(folderfilelist) - lenlastfolderfilelist, len(folderfilelist)),
                      flush=True)
                print("Find %d new file(s),%d file(s) in all" % (len(folderfilelist) - lenlastfolderfilelist, len(folderfilelist)),
                      flush=True)
            else:
                break
        if time1 == 0:
            ...
            printGreen("已发现下列重复文件\nFollowing duplicated file(s) found")
            try:
            ...
            printGreen(str(len(e)) + "file(s) in all.")
            name = "重复文件DuplicateFile" + time.strftime("%Y%m%d%H%M%S", time.localtime()) + ".xls"
            book.save(name)
            print('\n重复文件名写入成功，文件名为' + name)
            print('\nWriting the duplicated file\'s name successfully,the file name is' + name + '\n')
            f = str(input("是否要删除重复文件？[y|n]\nDo you want to delete the duplicated file(s)?[y|n]"))
            if f == "y":
        ...
        folder3list = []
        printGreen("请输入备份文件目录。\nPlease enter the backup file directory.")
        folder2 = str(input(""))
        time4 = 0
        while 1:
            time4 += 1
            ...
            if wannacontinue == "y":
                ...
                folder3 = str(input(""))
                if folder2 == folder3:
                    printDarkRed("备份文件目录不能和文件目录一样\nThe backup directory cannot be the same as the file directory.")
                elif folder2 in folder3:
                    printGreen("对不起，现在暂时无法做这个功能\nSorry,I cannot do that function now.")
                    print("\n", flush=True)
        ...
        if not thediffrenceoffolder2filelistandfolder3filelist:
            ...
            printGreen("The content in the file directory：" + str(folder3filelist) + ",and they all have backuped")
            print("\n")
        else:
            ...
            printGreen("There is(are) difference(s) between two directoryies.%s in difference，%d files in all hasn't benn backuped" % (
                thediffrenceoffolder2filelistandfolder3filelist, len(thediffrenceoffolder2filelistandfolder3filelist)))
            printGreen("您要复制不同文件吗？[y|n]\nDo you want to copy the different file(s)?[y|n]")
            cancopy = str(input(""))
            if cancopy == "y":
